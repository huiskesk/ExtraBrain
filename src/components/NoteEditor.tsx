        ...
        {(viewMode === "edit" || viewMode === "split") && (
          <div className={`flex-1 overflow-y-auto ${viewMode === "split" ? "border-r border-gray-200" : ""}`}>
            <textarea
              ...
              className="w-full h-full p-6 text-gray-800 leading-relaxed resize-none outline-none font-mono text-sm"
            />
          </div>
        )}
...
function PdfViewer({ note }: { note: Note }) {
  // For now, we'll show a placeholder. In a real app, we'd use a PDF viewer library
  return (
    <div className="flex-1 flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {isEditing ? (
          <textarea
            ...
            className="w-full h-full font-mono text-sm resize-none outline-none border border-gray-200 rounded-lg p-4"
          />
        ) : (
          <div
